...
def get_image_points(fname, inner_width, inner_height, on_calibrated_signal=None):
    ...
    if ret:
        ...
        img = cv2.drawChessboardCorners(
            img, (inner_width, inner_height), corners2, ret)
...
def calibrate_camera(images, inner_width, inner_height, on_calibrated_signal=None):
    ...
    for fname in images:
        corners = get_image_points(
            fname, inner_width, inner_height, on_calibrated_signal)
    ...
    return cv2.calibrateCamera(objpoints, imgpoints, image_size, None, distCoeffs=np.zeros(5),
                               flags=cv2.CALIB_ZERO_TANGENT_DIST) + (imgpoints, objpoints, file_names)
